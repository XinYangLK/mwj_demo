...
- (void)setupTitleButtons {
    ...
    if (self.allBtnWidth <= self.bounds.size.width) { // SGPageTitleView 静止样式
        ...
        for (NSInteger index = 0; index < titleCount; index++) {
            ...
            if ([self.titleArr[index] rangeOfString:@"进行中"].location !=NSNotFound ||
                [self.titleArr[index] rangeOfString:@"即将开始"].location !=NSNotFound) {
                
                btn.titleLabel.lineBreakMode = NSLineBreakByWordWrapping;
                NSString * textStr;
                if ([self.titleArr[index] rangeOfString:@"进行中"].location !=NSNotFound) {
                    textStr =@"进行中";
                }else if ([self.titleArr[index] rangeOfString:@"即将开始"].location !=NSNotFound) {
                    textStr =@"即将开始";
                }
                btn.titleLabel.attributedText =[NSMutableAttributedString getAttributedString:self.titleArr[index]
                                                                                 handleString:textStr
                                                                                handleStrFont:kFont_13
                                                                               handleStrColor:self.configure.titleColor
                                                                                    lineSpace:5];
