...
- (void)initAttributes {
    ...
    self.customTableHeight = MainHeight - StatusBarAndNavigationBarHeight -TabbarHeight-AdaptedHeight(44);
    self.cellHeight = 44;
...
- (CGSize)calculateStringSize: (NSString *)string {
    ...
    CGFloat w = ceil(size.width) + 2;
    CGFloat max = (MainWidth-(self.firstArray.count*50))/ self.firstArray.count;
    return CGSizeMake(w >= max ? max : w, size.height);
}
        ...
        for (NSInteger i = 0; i < self.numberOfColumn; i++) {
            if (i != tapIndex) {
                [self animateForIndicator:_currentIndicatorLayers[i] show:NO complete:^{
                    [self animateForTitleLayer:self.currentTitleLayers[i] indicator:nil show:YES complete:^{
                    }];
                }];
            }
        }
            ...
            case TFDropDownMenuStyleCustom: {
                ...
                if (_currentSelectColumn < _customViews.count) {
                    ...
                    if (view != nil) {
                        ...
                        viewHeight = viewHeight > _customTableHeight ? _customTableHeight : viewHeight;
            ...
            case TFDropDownMenuStyleCustom: {
                ...
                if (_currentSelectColumn < _customViews.count) {
                    ...
                    if (view != nil) {
                        ...
                        viewHeight = viewHeight > _customTableHeight ? _customTableHeight : viewHeight;
                        view.frame = CGRectMake(0, 0, self.bounds.size.width, viewHeight);
                        self.customScrollView.frame = CGRectMake(self.bounds.size.width, self.frame.origin.y + self.bounds.size.height, 0, viewHeight);
                        [self.customScrollView addSubview:view];
                        [self.superview addSubview:self.customScrollView];
                        [UIView animateWithDuration:_kAnimationDuration animations:^{
                            
                            self.customScrollView.frame = CGRectMake(self.frame.origin.x+AdaptedWidth(60), self.frame.origin.y + self.bounds.size.height, self.bounds.size.width-AdaptedWidth(60), viewHeight);
                        }];
                    } else {
                        self.leftTableView.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y + self.bounds.size.height, self.bounds.size.width, 0);
                        [self.superview addSubview:self.leftTableView];
            ...
            case TFDropDownMenuStyleCustom: {
                if (_currentSelectColumn < _customViews.count) {
                    ...
                    if (view != nil) {
                        ...
                        viewHeight = viewHeight > _customTableHeight ? _customTableHeight : viewHeight;
                        [UIView animateWithDuration:_kAnimationDuration animations:^{
                            self.customScrollView.frame = CGRectMake(MainWidth, self.frame.origin.y + self.bounds.size.height, 0, viewHeight);
                        } completion:^(BOOL finished) {
                            [self.customScrollView removeFromSuperview];
                            for (UIView *subView in [self.customScrollView subviews]) {
                                [subView removeFromSuperview];
