...
- (UIView *)buidlStarViewWithImageName:(NSString *)imageName isForegroundColor:(BOOL)isForegroundColor
{
    ...
    for (int i = 0; i < self.numberOfStar; i ++)
    {
        ...
        CGFloat space = self.space > 0 ? self.space : AdaptedWidth(2);
        
        imageView.frame = CGRectMake(i * (frame.size.width+space) / self.numberOfStar, 0, (frame.size.width-space*4) / self.numberOfStar, frame.size.height);
