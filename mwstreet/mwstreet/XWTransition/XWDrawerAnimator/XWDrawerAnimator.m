...
- (void)xw_setToAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    if (self.flipEnable) {
        fromTempView.frame = CGRectMake(0, StatusBarHeight + 20, MainWidth, MainHeight);
    }else{
        fromTempView.frame = CGRectMake(0, 0, MainWidth, MainHeight);
    }
    ...
    [UIView animateKeyframesWithDuration:self.toDuration delay:0.0 options:0 animations:^{
        if (weakSelf.flipEnable) {
            ...
            [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:1.0 animations:^{
                if (weakSelf.parallaxEnable) {
                    fromTempView.transform = weakSelf.vertical ? CGAffineTransformMakeTranslation(0, -distance) : CGAffineTransformMakeTranslation(-distance, 0);
                }
                fromTempView.transform = CGAffineTransformScale(fromTempView.transform, weakSelf.scaleRatio, weakSelf.scaleRatio);
            }];
        }
        [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:1.0 animations:^{
...
- (void)xw_setBackAnimation:(id<UIViewControllerContextTransitioning>)transitionContext{
    ...
    [UIView animateKeyframesWithDuration:self.backDuration delay:0.0 options:0 animations:^{
        if (weakSelf.flipEnable) {
            [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:0.35 animations:^{
                weakSelf.toFromTempView.frame = CGRectMake(0, 0, MainWidth, MainHeight);
                weakSelf.toFromTempView.layer.transform = [self _xw_stepOneTransform];
            }];
            [UIView addKeyframeWithRelativeStartTime:0.35 relativeDuration:0.65 animations:^{
                weakSelf.toFromTempView.layer.transform = CATransform3DIdentity;
            }];
            
        }else{
            [UIView addKeyframeWithRelativeStartTime:0.0 relativeDuration:1.0 animations:^{
                weakSelf.toFromTempView.transform = CGAffineTransformIdentity;
            }];
        ...
        if (![transitionContext transitionWasCancelled]) {
            ...
            toVC.view.hidden = NO;
            [weakSelf.toFromTempView removeFromSuperview];
