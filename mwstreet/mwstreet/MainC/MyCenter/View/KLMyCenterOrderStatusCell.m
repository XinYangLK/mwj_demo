...
@implementation KLMyCenterOrderStatusCell
...
- (void)setDataArray:(NSArray *)dataArray {
    ...
    NSInteger col = dataArray.count;    //这里只布局列
    ...
    for (int i = 0; i < count; i++) {
        ...
        NSString * img;
        if (self.type == 2) {
            img =@[@"daifukuan",@"daifahuo",@"yishouhuo",@"tuihuodan"][i];
        }else{
            img =[NSString stringWithFormat:@"icon_%d",i+1];
        }
        UIButton * button = [UIButton createBtnTitle:dataArray[i] titleColor:RGBSigle(51) font:kFont_13 imageName:img backgrounColor:[UIColor clearColor] SSImagePositionType:SSImagePositionTypeTop spacing:6 action:^(UIButton * _Nonnull button) {
            if (weakSelf.delegate && [weakSelf.delegate respondsToSelector:@selector(didOrderStatusBtnChick:)]) {
                [weakSelf.delegate didOrderStatusBtnChick:button];
            }
        }];
        button.tag = i + 10;
